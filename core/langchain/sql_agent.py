...
class MentalHealthSQLAgent:
    ...
    def _load_schema_info(self):
        ...
        try:
            schema_path = Path(__file__).parent.parent.parent / "data" / "schema_definitions" / "mental_health_schema.yaml"
    ...
    async def _get_database_connection(self):
        ...
        if self._db is None:
            try:
                ...
                logger.error("Failed to create database connection for SQL agent", error=str(e))
                raise
...
"""
            ...
            logger.info("Query analysis completed", question=user_question)
            
            # Step 2: Execute search using Supabase
            # For now, we'll do a simple text search
            search_results = await supabase_db.search_services_by_text(user_question, limit=10)
...
"""
            ...
            logger.error("Query processing failed", question=user_question, error=str(e))
            return {
                "status": "error",
                "error": str(e),
                "user_question": user_question

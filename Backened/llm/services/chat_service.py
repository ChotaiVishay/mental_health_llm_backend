...
from .flows.service_creation import prepare_payload
...
class MentalHealthChatService:
    ...
    async def process_message(
        ...
        try:
            ...
            logger.info("sid", sid=session_id)
            
            # === Handle one-shot service form submission (bypass Q&A) ===
            if isinstance(message, dict) and message.get("type") == "service_form":
                try:
                    # validate + normalize
                    payload = prepare_payload(message.get("data") or {})
                except Exception as e:
                    err = f"Some details look invalid: {e}"
                    # store a short note, not the whole form
                    await self._store_message(session_id, "assistant", err)
                    return {
                        "message": err,
                ...
                result = supabase_db.insert_service(payload)

                ok = result.get("status") == "success"
                msg = "Saved! The service has been added to the directory." if ok \
                    else f"Sorry, I couldn’t save this service ({result.get('error')}). Please try again."

                # (optional) store a short audit line; avoid saving full PII payload in chat log
                await self._store_message(session_id, "user", "[Service form submitted]")
                await self._store_message(session_id, "assistant", msg)
            ...
            # If no ongoing flow, check for "add_service" intent and start the flow
            if intent == "add_service":
                reply = "Great — let's add a new service to the directory.\n"
                await self._store_message(session_id, "assistant", reply)
                return {
                    "message": reply,
                    "session_id": session_id,
                    "services_found": 0,

...
class MentalHealthChatService:
        ...
        try:
            ...
            if USE_SIMPLE_AGENT:
                ...
                if result["status"] == "success":
                    ...
                    response_text = "I'm sorry, I'm having some technical difficulties right now."
                    response_data = {
                        "message": response_text,
                        "session_id": session_id,
                        "services_found": 0,
    ...
    async def _store_message(self, session_id: str, role: str, content: str):
        ...
        try:
            ...
            if result.data:
                logger.debug("Message stored successfully", session_id=session_id, role=role)
            else:
                logger.warning("Message storage returned no data", session_id=session_id)
                
        except Exception as e:
            logger.warning("Failed to store message (non-critical)", session_id=session_id, 
    ...
    async def get_conversation_history(self, session_id: str, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            messages = await supabase_db.query_table(
                "messages",
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            if USE_SIMPLE_AGENT:
                ...
                return {
                    "status": "healthy" if component_tests.get("openai", {}).get("status") == "success" and component_tests.get("database", {}).get("status") == "success" else "degraded",
                    "agent_type": "simple_test_agent",
                    "components": component_tests,
                    "ready_for_chat": component_tests.get("openai", {}).get("status") == "success" and component_tests.get("database", {}).get("status") == "success"
                }
            else:
                ...
                return {
                    "status": "healthy" if agent_status.get("status") == "success" and db_status.get("status") == "connected" else "degraded",
                    "agent_type": "sql_agent",
                    "sql_agent": agent_status,
                    "database": db_status,
                    "ready_for_chat": agent_status.get("status") == "success" and db_status.get("status") == "connected"
                }

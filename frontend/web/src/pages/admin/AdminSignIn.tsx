  ...
  return (
    <section className="auth-page" aria-labelledby="admin-auth-title">
      <div className="auth-card" role="dialog" aria-modal="true" aria-labelledby="admin-auth-title">
        ...
        <p className="muted">Sign in to access the admin console.</p>
        ...
        <div className="oauth-list" role="group" aria-label="Admin sign in with">
          <button
            ...
            disabled={loading}
            onClick={async () => {
              const res = await signIn('google', from);
              if (!res?.redirected) navigate(from);
            }}
          ...
          <button
            ...
            disabled={loading}
            onClick={async () => {
              const res = await signIn('github', from);
              if (!res?.redirected) navigate(from);
            }}
          ...
          <button
            ...
            disabled={loading}
            onClick={async () => {
              const res = await signIn('apple', from);
              if (!res?.redirected) navigate(from);
            }}
        ...
        {/* No external console; Admin lives in-app */}
     </div>
    </section>
  );
}
